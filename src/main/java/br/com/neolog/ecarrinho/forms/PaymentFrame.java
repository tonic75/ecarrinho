...
public class PaymentFrame extends JFrame {
	...
	public PaymentFrame() {
		...
		paymentWaitFrame.add(new JLabel(
				"Esperando pela confirmação do seu pagamento"));
	...
	private class Worker extends SwingWorker<Void, Void> {
		...
		protected Void doInBackground() throws Exception {
			...
			case 2:
				if (creditService.isDebtOk(agencyField.getText(),
						accField.getText())) {
					saveOrder();
				} else {
					showPaymentProblemOptionPane();
	...
	public void saveOrder() {
		try {
			userOrderService.save(new UserOrder(basketService.getBasket(),
					...
					addressArea.getText(), cardField.getText(), agencyField
							.getText(), accField.getText(),
